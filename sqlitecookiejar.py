...
class SQLiteCookieJar(FileCookieJar):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.filename is None:
            ...
            print "INFO : no filename argument specified. Your cookie jar will be in :"
            print "   ", self.filename
    ...
    def _save_cookie(self, cookie):
        ...
        with sqlite3.connect(self.filename) as con:
            # print "Saving cookie [domain:", cookie.domain, ", name :", cookie.name,  ", value:", cookie.value,"]"
    ...
    def _really_load(self):
        ...
        for cookie in res:
            ...
            # print "loaded cookie [domain: ", c.domain, ", name:", c.name, ", value:", c.value, "]"
        ...
        with sqlite3.connect(self.filename) as con:
            ...
            if len(res) == 0:
                ...
                if table_name != "cookie":
                    # Case #3
                    raise AttributeError("The specified database has one table named '%s', but the tabled should be named"
                                         " 'cookie'. Please provide a valid database." % table_name)
                    ...
                    if len(columns.difference(expected_columns)) > 0:
                        raise AttributeError("The specified database has a table named 'cookie', but the column names"
                                             "do not match. Expected : %s with %s elements; got %s with %s elements "
                                             "instead." %
                                             (list(expected_columns), len(expected_columns), list(columns), len(columns)))

            else:
                # Case #4
                raise AttributeError("The specified database has more than one table. Please provide a valid database.")